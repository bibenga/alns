...
import (
	...
	"github.com/bibenga/alns/internal/math"
)
...
var _ alns.OperatorSelectionScheme = &RouletteWheel{}
...
func (s *RouletteWheel) Select(rnd *rand.Rand, best, current alns.State) (int, int, error) {
	if s.opCoupling != nil {
		// select destroy operator
		dIdx := math.WeightedRandomIndex(rnd, s.dWeights)
		...
		rIdx := s.coupledRIdcs[math.WeightedRandomIndex(rnd, s.coupledRWeights)]

		return dIdx, rIdx, nil
	} else {
		dIdx := math.WeightedRandomIndex(rnd, s.dWeights)
		rIdx := math.WeightedRandomIndex(rnd, s.rWeights)
		return dIdx, rIdx, nil
	}
}

func (s *RouletteWheel) Update(candidate alns.State, deleteOpIndx int, repairOpIndx int, outcome alns.Outcome) error {
